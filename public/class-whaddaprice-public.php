...
class Whadda_shortpublic {
  ...
  function jb_shortcode_de_contenido($idp) {
    ...
    $this->test = new Whadda_css();
    $this->test->whadda_styles_method($idp['id']);
    ...
    $font = get_post_meta($idp["id"], 'whadda_fonts', true);
    ...
    $reg_font = $prefix . 'font_style' . $idp["id"];
    ...
    if ($layout == 1) {
      echo
      '<div class="wrap whadda_font f' . $idp['id'] . '">';
      /*
        '<div class="miswitch">'.
...
class Whadda_css {
  ...
  function whadda_styles_method($id) {
   ...
   wp_enqueue_style(
            'whadda-custom-css',
            plugin_dir_url(__FILE__) . 'css/whaddaprice-admin.css'
    );
    $font = get_post_meta($id, 'whadda_fonts', true); //get_theme_mod( '#ffffff' ); //E.g. #FF0000
    $custom_css = "
                @font-face{
                        font-family: f{$id};
                          src:url({$font});
